                ...
                <tbody>
                  {watchlist.length > 0 ? (
                    watchlist.map((asset, index) => (
                      <AssetRow key={asset.id || `asset-${index}`} asset={asset} index={index} refreshWatchlist={displayWatchlist} />
                    ))
                  ) : (
                    <tr>
...
const AssetRow = ({ asset, index, refreshWatchlist }) => {
  ...
  const handleConfirmRemove = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        refreshWatchlist(); // Refresh the list after removal
      } else {
